...
`web connect` はブラウザをログインさせてクライアントへリダイレクトする使い捨てリンクを表示します。リンクは `[web] invite_ttl_secs` の経過後に失効します。`--url` はそのリンクに限り設定済みの `[web] public_url` を上書きします。`web sessions` はブラウザセッションと接続中の数を一覧します。`web disconnect` はセッションを失効させ、接続を即座に閉じます。いずれも `[web] enabled = true` が必要です。[ブラウザクライアント](/docs/web-client/) を参照してください。
