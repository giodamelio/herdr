...
```bash
herdr web connect [--url ORIGIN] [--json]
herdr web sessions [--json]
herdr web disconnect <session>
```

`web connect` prints a single-use link that logs a browser in and redirects it to the client; the link expires after `[web] invite_ttl_secs`. `--url` overrides the configured `[web] public_url` for that link only. `web sessions` lists browser sessions with their live connection counts. `web disconnect` revokes a session and closes its connections immediately. All three require `[web] enabled = true`. See [Browser client](/docs/web-client/).
