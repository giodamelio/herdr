...
`web connect` 打印一个一次性链接，用于登录浏览器并重定向到客户端；链接在 `[web] invite_ttl_secs` 之后失效。`--url` 仅为该链接覆盖已配置的 `[web] public_url`。`web sessions` 列出浏览器会话及其活动连接数。`web disconnect` 撤销会话并立即关闭其连接。三者都需要 `[web] enabled = true`。参见[浏览器客户端](/docs/web-client/)。
